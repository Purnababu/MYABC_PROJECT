...
public interface IEngineerRepository extends JpaRepository<Engineer, Integer> {
	...
	public List<Complaint> getResolvedComplaintsByDate(@Param("engineer") Engineer engineer, @Param("date") LocalDate date);

	@Query("SELECT c FROM Complaint c where  c.engineer = ?1")
	public List<Complaint> getResolvedComplaints(@Param("engineer") Engineer engineer);

	@Query("SELECT c FROM Complaint c where  c.engineer = ?1")
	public List<Complaint> getComplaints(@Param("engineer") Engineer engineer, @Param("complaintName") String complaintName);
